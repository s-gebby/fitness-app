  ...
  return (
    <div className='flex flex-col gap-4'>
      <div className='flex items-center gap-2 justify-center'>
        <p className='text-3xl sm:text-4xl md:text-5xl font-semibold text-sky-400'>{index}</p>
...
export default function Generator(props) {
  ...
  return (
    <SectionWrapper header={"generate your workout"} title={['It\'s', 'Huge', 'o\'clock']}>
        ...
        {Object.keys(WORKOUTS).map((type, typeIndex) => {
          return (
            <button onClick={() => {
              setMuscles([])
              setPoison(type)
            }} className={'bg-slate-950 border duration-200 px-4 border-sky-300 py-4 rounded-lg hover:border-sky-800' + (type === poison ? 'border-sky-600' : 
              ' border-sky-400'
            )} key={typeIndex}>
      ...
      <div className='bg-slate-950 border border-solid border-sky-400 rounded-lg flex flex-col'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
        {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
          return (
            <button onClick={() => {
              setGoal(scheme)
            }} className={'bg-slate-950 border duration-200 border-sky-300 py-4 rounded-lg hover:border-sky-800' + (scheme === goal ? 'border-sky-600' : 
              ' border-sky-400'
            )} key={schemeIndex}>
